...
export async function get<T>(
  ...
  config.params = { t: new Date().getTime(), ...config?.params, ...newFilters };
...
export async function del<T>(uri: string, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> {
  ...
  config.params = { t: new Date().getTime(), ...config?.params };
...
export type ErrorHandlerParams = { title?: string; message?: string; section?: string };
...
export function handleError(params?: ErrorHandlerParams) {
  return (err?: AxiosError<{ error: string }>) => {
    ...
    dialog.alert({ message: errMessage, title: params?.title || 'Failed' });
  };
}
